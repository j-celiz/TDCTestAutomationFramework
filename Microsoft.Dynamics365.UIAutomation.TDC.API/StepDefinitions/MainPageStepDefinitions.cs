...
namespace Microsoft.Dynamics365.UIAutomation.TDC.StepDefinitions
{
    ...
    public class MainPageStepDefinitions : BaseStepDefinitions
    {
        ...
        [Given(@"the API endpoint for category details ""(.*)"" is available")]
        public async Task GivenTheApiEndpointForCategoryDetailsIsAvailableAsync(string endpoint)
        {
            XrmApp = new XrmApp(client);
            await BusinessCentral.TestAPI(endpoint);
        }
